 ...
 */
public class RemoveClientActivity extends ListActivity {
    ...
    public void onListItemClick(ListView parent, View view, int position, long id) {
        if (!clients[0][position].equals("No clients")) {
            removeClient(clients[3][position]);
            this.finish();
        }
    }

    public void removeClient(String num_client) {
        AsyncHttpClient client = new AsyncHttpClient();
        RequestParams params = new RequestParams();
        ...
        Gson gson = new GsonBuilder().create();
        params.put("removeClientJSON", gson.toJson(wordList));

        // On envoie username / password pour vérifier si lesidentifiants sont bons selon la bdd en ligne
        client.post("http://www.pierre-ecarlat.com/newSql/removeclient.php", params, new AsyncHttpResponseHandler() {
            @Override
            public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
                String response = null;
                clients = null;
