...
public class MyBDD {
    ...
    private static final String COL_LINK_COMPT = "COMPTEUR";
...
/** FONCTIONS RECHERCHE ***************************************************************************/
    public int getNbCompaniesFromPeople(int peopleId) {
        Cursor c = bdd.rawQuery("SELECT " + COL_COMPANY_NAME + " FROM " + TABLE_COMPANY
                + " WHERE " + COL_COMPANY_ID + " IN (SELECT " + COL_LINK_IDCOMP + " FROM " + TABLE_LINK + " WHERE " + COL_LINK_IDPEOP + " = " + peopleId + ")", null);
        return c.getCount();
    }

    public String[] getAllCompaniesJoinedByUser(int peopleId, int nbComp) {
        int compteur = 0;
        Cursor c = bdd.rawQuery("SELECT " + COL_COMPANY_NAME + " FROM " + TABLE_COMPANY
                + " WHERE " + COL_COMPANY_ID + " IN (SELECT " + COL_LINK_IDCOMP + " FROM " + TABLE_LINK + " WHERE " + COL_LINK_IDPEOP + " = " + peopleId + ")", null);
        String s[] = new String[nbComp];
    ...
    public String[] getCompaniesJoinedByPeople(int peopleId) {
        String companies[];
        int nbComp = getNbCompaniesFromPeople(peopleId);

        companies = getAllCompaniesJoinedByUser(peopleId, nbComp);

        return companies;
    }

    public int getNbReductionsFromPeopleAndCompany(int peopleId, int companyId) {
        Cursor c = bdd.rawQuery("SELECT " + COL_REDUCTION_TYPE + " FROM " + TABLE_REDUCTION
                + " WHERE " + COL_REDUCTION_ID + " IN (SELECT " + COL_LINK_IDREDU + " FROM " + TABLE_LINK + " WHERE " + COL_LINK_IDPEOP + " = " + peopleId + " AND " + COL_LINK_IDCOMP + " = " + companyId + ")", null);
        return c.getCount();
    }

    public String[] getAllReductionsAllowedByUserAndCompany(int peopleId, int companyId, int nbRedu) {
        int compteur = 0;
        Cursor c = bdd.rawQuery("SELECT " + COL_REDUCTION_TYPE + " FROM " + TABLE_REDUCTION
                + " WHERE " + COL_REDUCTION_ID + " IN (SELECT " + COL_LINK_IDREDU + " FROM " + TABLE_LINK + " WHERE " + COL_LINK_IDPEOP + " = " + peopleId + " AND " + COL_LINK_IDCOMP + " = " + companyId + ")", null);
    ...
    public String[] getReductionsAllowedFromPeopleAndCompany(int peopleId, String company_name) {
        String reductions[];
        int companyId = getCompanyIdWithName(company_name);
        if(companyId == -1) { return null; }

        int nbRedu = getNbReductionsFromPeopleAndCompany(peopleId, companyId);

        reductions = getAllReductionsAllowedByUserAndCompany(peopleId, companyId, nbRedu);

        return reductions;
    }

    public int getNbReductionsProvided(int companyId) {
        Cursor c = bdd.rawQuery("SELECT " + COL_REDUCTION_TYPE + " FROM " + TABLE_REDUCTION
                + " WHERE " + COL_REDUCTION_ID + " IN (SELECT " + COL_LINK_IDREDU + " FROM " + TABLE_LINK + " WHERE " + COL_LINK_IDCOMP + " = " + companyId + ")", null);
        return c.getCount();
    }

    public String[] getAllReductionsProvided(int companyId, int nbRedu) {
        int compteur = 0;
        Cursor c = bdd.rawQuery("SELECT " + COL_REDUCTION_TYPE + " FROM " + TABLE_REDUCTION
                + " WHERE " + COL_REDUCTION_ID + " IN (SELECT " + COL_LINK_IDREDU + " FROM " + TABLE_LINK + " WHERE " + COL_LINK_IDCOMP + " = " + companyId + ")", null);
        String s[] = new String[nbRedu];
    ...
    public String[] getAllReductionsProvidedByCompany(int companyId) {
        String reductions[];
        int nbRedu = getNbReductionsProvided(companyId);

        reductions = getAllReductionsProvided(companyId, nbRedu);

        return reductions;
    }

    public int getNbPeopleJoined(int companyId) {
        Cursor c = bdd.rawQuery("SELECT " + COL_PEOPLE_NAME + ", " + COL_PEOPLE_FIRST_NAME + " FROM " + TABLE_PEOPLE
                + " WHERE " + COL_PEOPLE_ID + " IN (SELECT " + COL_LINK_IDPEOP + " FROM " + TABLE_LINK + " WHERE " + COL_LINK_IDCOMP + " = " + companyId + ")", null);
        return c.getCount();
    }

    public String[] getAllPeopleJoined(int companyId, int nbPeop) {
        int compteur = 0;
        Cursor c = bdd.rawQuery("SELECT " + COL_PEOPLE_NAME + ", " + COL_PEOPLE_FIRST_NAME + " FROM " + TABLE_PEOPLE
                + " WHERE " + COL_PEOPLE_ID + " IN (SELECT " + COL_LINK_IDPEOP + " FROM " + TABLE_LINK + " WHERE " + COL_LINK_IDCOMP + " = " + companyId + ")", null);
        ...
        do
        {
            s[compteur] = c.getString(0) + " " + c.getString(1);
            compteur++;
        } while(c.moveToNext());
    ...
    public String[] getAllPeopleInCompany(int companyId) {
        String people[];
        int nbPeop = getNbPeopleJoined(companyId);

        people = getAllPeopleJoined(companyId, nbPeop);
