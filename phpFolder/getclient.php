<?php
	...
	if($db->checkIfClientIsOkWithNumClient($data[0]->serial_number)) {
		$b["dataOk"] = 'yes';
		
		if($db->checkIfTruePeople($data[0]->connected_people, $data[0]->serial_number)) {
			$b["truePeople"] = 'yes';
			
			$res = $db->getClientWithNumClient($data[0]->serial_number);
